...
import { getAllBill } from "../../../api/apiBill";
import { useNavigate } from "react-router-dom";
import { CheckCircleIcon, FileText } from "lucide-react";
import { jsPDF } from "jspdf"; // Import jsPDF
import './all-bill.css';  // Import CSS file
...
const AllBill = () => {
  ...
  const filterBills = () => {
    ...
    if (startDate) {
      filtered = filtered.filter(
        (bill) => new Date(bill.createdTime) >= new Date(startDate)
      );
    }
    if (endDate) {
      filtered = filtered.filter(
        (bill) => new Date(bill.createdTime) <= new Date(endDate)
      );
    }
  ...
  const exportToPDF = () => {
    ...
    const header = `Bill Report (${startDate || "All time"} - ${
      endDate || "Present"
    })\n\n`;
